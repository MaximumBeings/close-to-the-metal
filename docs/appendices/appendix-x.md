# Appendix X — libtorch: The C++ API for Production Inference
...
## X.1 What libtorch Is and When to Use It
...
## X.2 Obtaining libtorch

### X.2.1 Pre-built download
...
### X.2.2 Extracting from an existing PyTorch install
...
## X.3 Building libtorch Projects with CMake

### X.3.1 Minimal CMakeLists.txt
...
### X.3.3 Linking against CUDA extensions
...
## X.4 Tensor Operations in C++

### X.4.1 Creating tensors
...
### X.4.2 Device and dtype options
...
## X.5 Loading and Running Models

### X.5.1 Loading a TorchScript model
...
### X.5.3 Named module access and parameter inspection
...
## X.7 Custom C++ Operations

### X.7.1 Writing a custom ATen op in C++
...
### X.7.2 Exposing C++ ops to Python via pybind11
...
## X.8 CUDA Integration in libtorch

### X.8.1 Device and stream management
...
### X.8.3 Pinned memory for fast H2D transfer
...
## X.9 Memory Management in C++

### X.9.1 The caching allocator
...
### X.9.2 Sharing storage: views without copies
...
## X.10 Building a Complete C++ Inference Server
...
// Build: see CMakeLists.txt in §X.3
...
## X.11 Performance Patterns in libtorch

### X.11.1 Avoiding unnecessary copies
...
### X.11.2 Operator fusion via torch::jit::optimize_for_inference
...
### X.11.3 Benchmarking C++ inference
...
## X.12 libtorch vs Python PyTorch: When to Switch
...
## X.13 Worked Example X.1 — Porting a Python Sampler to C++
...
## X.14 Test Harness — libtorch Concepts (Pure Python)
...
def rms_norm_python(x: torch.Tensor, weight: torch.Tensor, eps: float = 1e-5):
    """Python reference for the C++ rms_norm kernel (§X.7)."""
...
def top_k_sample_python(logits: torch.Tensor, k: int = 50,
    ...
    """Python equivalent of the C++ top_k_sample function (§X.10)."""
    logits = logits / temperature
...
def sample_top_p_python(logits: torch.Tensor, p: float = 0.9,
    ...
    """Python equivalent of the C++ sample_top_p function (§X.13)."""
    probs = torch.softmax(logits / temperature, dim=-1)
...
def test_column_parallel_simulation():
    """Simulate column-parallel linear (§X.5 C++ pattern, verified in Python)."""
...
## X.15 Quick-Reference: Python → C++ API Cheatsheet
...
*Cross-references: Appendix W (PyTorch Python API), Appendix J (CUDA C++ Introduction),
Appendix G (C++ Build Patterns), Appendix Q (CUTLASS), Chapter 8.5 (CUDA Graphs),
