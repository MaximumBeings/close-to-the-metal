# Code L — TurboQuant: Online Vector Quantization for KV Cache Compression

> Companion code for **Appendix L**. All numerical assertions verify the worked examples in the appendix exactly.
...
All numerical results match the worked examples in Appendix L exactly.
...
# ---------------------------------------------------------------------------
# Section 1 — Lloyd-Max Codebooks (Appendix L §L.2.3)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Section 2 — Hadamard Rotation (Appendix L §L.2.4, §L.10.2)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Section 3 — PolarQuant Encode / Decode (Appendix L §L.2.4)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Section 4 — QJL Residual Encoding / Correction (Appendix L §L.4)
# ---------------------------------------------------------------------------
...
def verify_example_l1():
    """
    Appendix L §L.3 — Worked Example L.1: PolarQuant 3-bit, d=4.
    ...
    print("\n✓ Worked Example L.1 verified.\n")


def verify_example_l2():
    """
    Appendix L §L.5 — Worked Example L.2: QJL residual correction.
    ...
    print("\n✓ Worked Example L.2 verified.\n")


def verify_example_l3():
    """
    Appendix L §L.6 — Worked Example L.3: Full write/read cycle, d=128.
    ...
    print("\n✓ Worked Example L.3 verified.\n")


def verify_example_l4():
    """
    Appendix L §L.7 — Worked Example L.4: TurboQuant vs INT8 error comparison.
    """
    print("=" * 68)
...
def vllm_integration_sketch():
    ...
    """
    print("=" * 68)
    print("vLLM INTEGRATION SKETCH (Appendix L §L.9)")
    print("=" * 68)
...
if __name__ == "__main__":
    print("\nTurboQuant Demo — Appendix L Companion Code")
 ...
 * Implements PolarQuant (Stage 1) + QJL residual correction (Stage 2).
 *
 * All numerical assertions verify the worked examples in Appendix L.
 *
 * Build:
...
// ---------------------------------------------------------------------------
// Section 4 — QJL Residual Encode / Correct (Appendix L §L.4)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Section 9 — Hadamard Efficiency Note (Appendix L §L.10.2)
// ---------------------------------------------------------------------------

void print_hadamard_efficiency_note() {
    std::cout << std::string(68, '=') << "\n";
    std::cout << "HADAMARD EFFICIENCY (Appendix L §L.10.2)\n";
    std::cout << std::string(68, '=') << "\n\n";
    std::cout << "For d=128:\n";
...
int main() {
    std::cout << "\nTurboQuant Demo — Appendix L Companion Code (C++)\n";
...
| Function / Struct | Appendix L Section | What It Verifies |
|---|---|---|
| `lloyd_max_codebook` / `get_codebook` | §L.2.3 | Codebook tables for 2, 3, 4 bits |
| `hadamard_matrix` / `make_hadamard` | §L.2.4, §L.10.2 | Normalized Hadamard rotation |
| `polar_quant_encode` / `decode` | §L.2.4 | PolarQuant σ, rotation, quantize, reconstruct |
| `qjl_encode` / `qjl_correct` | §L.4 | Sign bits, unbiased logit correction |
| `turboquant_encode` / `attention_logit` | §L.2–L.4 | Full two-stage pipeline |
| `compression_analysis` | §L.6 | Memory layout and compression ratios |
| `verify_example_l1` | §L.3 | σ=0.8173, codes=[4,5,1,6], MSE≈0.0288 |
| `verify_example_l2` | §L.5 | Signs=[-1,-1,+1,+1], 25× error reduction |
| `verify_example_l3` | §L.6 | 52 bytes/vector at d=128, 4.92× compression |
| `verify_example_l4` | §L.7 | INT8 RMSE=0.004 vs TQ3 RMSE≈0.016, 4.9× |
...
!!! note "Production vs. Demo"
    ...
    Transform (FWHT) sketch used in llama.cpp (§L.10.2). For vLLM, see the Triton kernel
